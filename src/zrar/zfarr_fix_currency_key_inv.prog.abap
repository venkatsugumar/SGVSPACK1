* &---------------------------------------------------------------------*
*& Report ZFARR_FIX_CURRENCY_KEY_INV
*&---------------------------------------------------------------------*
*&
*&---------------------------------------------------------------------*
REPORT zfarr_fix_currency_key_inv.
...
FORM sel_invoice_from_db.
  ...
  SELECT pob_id alloc_amt_curk INTO  TABLE gt_pob_data
  FROM farr_d_pob
    WHERE pob_id       IN s_pob
    AND   company_code IN s_bukrs
    AND   contract_id  IN s_con
    AND   ( alloc_amt_curk NE space ).
 ...
 IF lt_sel_pob_id IS NOT INITIAL.
...
FORM add_select_counter_to_log.
  MESSAGE i000(fb)
  WITH gv_counter
  'Number of FARR_D_INVOICE entries without currency key'
  INTO gv_msg_str.
